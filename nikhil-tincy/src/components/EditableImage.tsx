...
export const EditableImage: React.FC<EditableImageProps> = ({ 
  ...
  const { isLoading, updateContent, content } = useContent();
  // You may want to use useAuth or similar for isAuthenticated
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      updateContent(path.split('.')[0] as any, { [path.split('.').slice(1).join('.')]: publicUrl });
      setIsOpen(false);
